...
/**
 * Custom Binary Min-Heap implementation from scratch in Java using an array.
 *
 * Core Concepts:
 * - A Binary Min-Heap is a complete binary tree where the key at the root is the minimum
 ...
 * - poll (extractMin):  O(log N) - Replace root with last element and sift/bubble down
 * - size / isEmpty:     O(1)
 *
 * Space Complexity:
...
public class MinHeap {
    ...
    /**
     * Constructs an empty Min-Heap with an initial capacity.
     *
    ...
    /**
     * Retrieves the minimum element (at the root) without removing it.
     *
     * @return minimum element stored in the heap
    ...
    public int peek() {
        ...
        // In a min-heap, the minimum value is always at index 0
        return heap[0];
    }
     ...
     * 3. Decrement size.
     * 4. Call heapifyDown() to sink the new root to its correct position, restoring min-heap property.
     *
    ...
    public int poll() {
        ...
        int minItem = heap[0];           // Minimum item to return
        heap[0] = heap[size - 1];        // Move last element to root
        size--;                          // Shrink active heap size
        heapifyDown();                   // Restore min-heap ordering downwards
        return minItem;
    }
    ...
    public void insert(int item) {
        ...
        heapifyUp();                     // Restore min-heap ordering upwards
    }

    /**
     * Restores min-heap property upwards from the last inserted element.
     * Compares child with its parent. While child < parent, swap them.
     */
    private void heapifyUp() {
        int index = size - 1; // Start from the newly added leaf element

        // Bubble up while node has a parent and node's value is smaller than parent's value
    ...
    /**
     * Restores min-heap property downwards starting from the root.
     * Compares parent with its children. Swaps with the smaller child until
    ...
    private void heapifyDown() {
        ...
        while (hasLeftChild(index)) {
            // Assume left child is the smaller child initially
            int smallerChildIndex = getLeftChildIndex(index);

            // If right child exists and is even smaller than left child, choose right child
            if (hasRightChild(index) && rightChild(index) < leftChild(index)) {
                smallerChildIndex = getRightChildIndex(index);
            }

            // If current element is already smaller than or equal to smallest child, heap property holds
            if (heap[index] <= heap[smallerChildIndex]) {
                break;
            } else {
                // Otherwise swap with the smaller child and continue sinking downwards
                swap(index, smallerChildIndex);
            }

            index = smallerChildIndex; // Move pointer down
        }
    }
    ...
    /**
     * Example driver demonstrating Min-Heap insertion, peek, and extraction.
     */
    public static void main(String[] args) {
        ...
        // Root should contain minimum element (5)
        System.out.println("Min element (peek): " + minHeap.peek()); // Expected: 5

        // Extracting elements one by one should yield ascending sorted order (Heap Sort)
        System.out.println("Extracted elements in sorted order:");
        while (!minHeap.isEmpty()) {
            System.out.print(minHeap.poll() + " "); // Expected: 5 10 15 20 30
        }
        System.out.println();
    }
}